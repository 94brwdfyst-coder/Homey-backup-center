...
async function fixture(language='nl'){
 ...
 const app=new sandbox.module.exports();app.log=()=>{};app.error=()=>{};app.homey={settings:{get:k=>structuredClone(state[k]),set:(k,v)=>state[k]=structuredClone(v),unset:k=>delete state[k]},i18n:{getLanguage:()=>language},api:{getLocalUrl:async()=> 'http://example.invalid'},app:{manifest:{version:'0.3.27'}},setInterval:()=>1,clearInterval:()=>{},notifications:{createNotification:async()=>{}}};
 await app.onInit();await new Promise(r=>setImmediate(r));return {app,state,data,calls,client};
}
