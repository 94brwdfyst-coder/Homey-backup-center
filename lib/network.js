...
const messages={
 ...
 HOST_KEY:'SFTP host key did not match the configured SHA256 fingerprint.',
...
function networkError(code){return Object.assign(new Error(messages[code]||messages.IO),{code:messages[code]?code:'IO'});}
function runWorker(target,filename,body){
 return new Promise((resolve,reject)=>{
  ...
  worker.once('message',m=>m.ok?finish(null,m):finish(networkError(m.code)));
...
function normalize(input,previous){
 ...
 const fingerprint=type==='sftp'?String(input.fingerprint||'').trim():'';
 if(type==='sftp' && !/^SHA256:[A-Za-z0-9+/]{43}=?$/.test(fingerprint))throw Error('Enter the server SHA256 host-key fingerprint, obtained from its administrator.');
...
class NetworkDestinations {
 ...
 async test(id){
  ...
  try {return await this.run(target,null,null);}catch(e){throw networkError(e.code);}finally{this.testing=false;}
 }
 async backup(id){
